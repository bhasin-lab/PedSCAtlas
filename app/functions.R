...
#' @param en pathway enrichment values
#' @param coords cells with CellType and Group features
#' @param group_ribbon column to check for Group features
#' @param g1 group 1 of statistical comparison
#' @param g2 group 2 of statistical comparison
#' @param paths names of pathways to plot
...
plotSurv = function(os, name = "gene", type = "All") {
    ...
    nLow = length(which(os$group == 'Low'))
    nHigh = length(which(os$group == 'High'))
    ...
    p = survminer::ggsurvplot(surv, data = os,
        ...
        pval = TRUE,
        xlab = "Time (Months)",
        legend.labs = c(paste0('Low (n = ', nLow, ' )'), paste0('High (n = ', nHigh, ' )'))
